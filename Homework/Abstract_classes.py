...
class Triangle(Figure):
    ...
    def __init__(self, a_side, b_side, c_side, size):
        ...
        """
        if isinstance(a_side, int | float) and isinstance(b_side, int | float) and isinstance(c_side,
    ...
    def perimeter(self):
        return f'Perimeter of triangel: {self.a_side + self.b_side + self.c_side} {self.size}'

    def area(self):
        p = (self.a_side + self.b_side + self.c_side) / 2
        return f'Area of triangle: {round(math.sqrt(p * (p - self.a_side) * (p - self.b_side) * (p - self.c_side)), 2)} {self.size}^2'
...
class Circle(Figure):
    ...
    def __init__(self, radius, size):
        """
        :param radius: a side of triangle -> int|float
        :param size: size of triangle -> str
        """
